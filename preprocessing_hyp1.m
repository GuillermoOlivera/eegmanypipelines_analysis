...
% all_ERPs = zeros(400, num_subjects, num_conditions, num_electrodes); % not used
...
for subject = subjects_to_use
    ...
    for electrode = electrodes_to_use
        ...
        ERP_matrix_manmade = zeros(number_of_trials, length_segment); % empty matrix for each segment, each segment is 400ms
        ERP_matrix_natural = zeros(number_of_trials, length_segment); % empty matrix for each segment, each segment is 400ms
        ...
        for condition = (1:2)
            ...
            for trial = (1:number_of_trials) % loop through all trials
                ...
                if (any(all_triggers(trial,1) == conditions_to_use))
                    ...
                    stimulus_time_s = stimulus_time_ms / 1000;
                    stimulus_timepoint = floor(stimulus_time_s / (1/sample_rate_hz));
                    channel_segment = data(electrode,stimulus_timepoint - pre_stimulus:stimulus_timepoint + post_stimulus); % original_ not electrode but channel!!
                    channel_segment = channel_segment - mean(channel_segment(1 : pre_stimulus));
                    
                    vEOG_segment = vEOG_data(stimulus_timepoint - pre_stimulus:stimulus_timepoint + post_stimulus); % get EOG segment
                    vEOG_segment = vEOG_segment - mean(vEOG_segment(1 : pre_stimulus)); % baseline correct
                    
                    hEOG_segment = hEOG_data(stimulus_timepoint - pre_stimulus:stimulus_timepoint + post_stimulus);
        ...
        trial_data_summary.erp_manmade_mean = erp_manmade_mean - mean(erp_manmade_mean(1: pre_stimulus));    
        trial_data_summary.erp_natural_mean = erp_natural_mean - mean(erp_natural_mean(1: pre_stimulus));
