...
if isOctave
    folder = '/mnt/research/EMP_data/EMP_data/eeg_brainvision';
    ...
    folder_analysed_data = '/media/cygnuseco/ext4_files/research/EMP_data/EMP_data/eeg_brainvision/_analysed_hyp2abc'; 
else
...
for subject = subjects_to_use
    ...
    for electrode = selected_electrodes
        ...
        for id = 1:size(event_types, 1)
            event_type = event_types(id, :);
            summary.(subject_id_str).(event_type).(electrode_id_str) = summary.(subject_id_str).(event_type).(electrode_id_str) / total_electrodes;
        end
    end
